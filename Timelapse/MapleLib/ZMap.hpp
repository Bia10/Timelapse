...
namespace Timelapse
{
	...
	public:
		value_type* GetAt(key_type const& key, value_type* value) {
			if (!this->_m_apTable) return nullptr;
	...
	private:
		template <typename T>
		static unsigned int CalculateDigest(T const& key) {
			return _rotr(key, 5);
		}
	...
	private:
		...
		unsigned int _m_uTableSize = 0;
		unsigned int _m_uCount = 0;
		unsigned int _m_uAutoGrowEvery128 = 0;
		unsigned int _m_uAutoGrowLimit = 0;
	};
